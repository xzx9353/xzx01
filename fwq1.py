...
# 用于并行执行任务的线程池
executor = ThreadPoolExecutor(max_workers=50)  # 调整为 50 以适应你的需求
    ...
    try:
        ...
        with delete_lock:
            ...
            if current_count == url_count:
                xzx = command[command.index('--audio_in') + 1]
                # 删除文件
                if os.path.exists(xzx):
                    os.remove(xzx)
                    print(f"文件 {xzx} 已删除")
                else:
                    print(f"文件 {xzx} 不存在")
            #
                # 删除目录中的文件
                filename = os.path.splitext(os.path.basename(xzx))[0]
                ...
                if os.path.exists(output_left):
                    ...
                    print(f"文件 {output_left} 已删除")
                    print(f"文件 {output_right} 已删除")
                else:
                    print(f"文件 {output_left} 不存在")
                    print(f"文件 {output_right} 不存在")
...
def ASR_func():
    ...
    if 'url' in data:
        ...
        for idx, url in enumerate(urls):
            future = executor.submit(get_file_from_url, url, UPLOAD_FOLDER)
            ...
            # 每个URL下载完后执行命令
            command = ["python", "cs3.py", "--audio_in", result]
            executor.submit(run_command, command, result, url_count, idx + 1)
